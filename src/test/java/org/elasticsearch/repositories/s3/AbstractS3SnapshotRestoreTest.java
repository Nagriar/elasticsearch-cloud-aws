...
abstract public class AbstractS3SnapshotRestoreTest extends AbstractAwsTest {
   ...
   /* @Test
    ...
    @Test
    public void testClientEncryption() throws NoSuchAlgorithmException, NoSuchProviderException {
	KeyPairGenerator keyGen = KeyPairGenerator.getInstance("RSA");
	SecureRandom random = SecureRandom.getInstance("SHA256", "SUN");
	...
	PutRepositoryResponse putRepositoryResponse = client.admin().cluster().preparePutRepository("test-repo")
		.setType("s3").setSettings(ImmutableSettings.settingsBuilder()
			.put("base_path", basePath)
			.put("chunk_size", randomIntBetween(1000, 10000))
			.put("client_side_encryption_public_key", pair.getPublic())
			.put("client_side_encryption_private_key", pair.getPrivate())
		).get();
	assertThat(putRepositoryResponse.isAcknowledged(), equalTo(true));
